...
// import { usePostContext, PostContextType } from '../contexts/PostContext'
...
const Post = ({ post }: Props): ReactElement => {
  ...
  // const { isFavorite, addToFavorites, removeFromFavorites } = usePostContext() as PostContextType
  // const favorite = isFavorite(post.id)

  // const onFavoriteClick = (e: MouseEvent): void => {
  //   e.preventDefault()
  //   if (favorite) {
    ...
    <article className='posts__card'>
      ...
      <div className='posts__post-content'>
        ...
        <div className='posts__footer'>
          ...
          <Link to={`/posts/${post.id}`} className='posts__readmore-link'>
            Читать далее
          </Link>
        </div>
      </div>
